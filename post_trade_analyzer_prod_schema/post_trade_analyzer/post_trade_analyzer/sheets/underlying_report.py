...
class UnderlyingReportSheet(ttk.Frame):
    """
    Underlying - Report (MULTI-DAY AGGREGATED):
      - Select multiple days
      - Aggregate into one row per underlyingName
      - Source is the precomputed (date, underlying) EOD table
      - Columns:
          underlyingName, feesCum, PnLVonDeltaCum, PremiaCum, Total, trades, portfolio
    ...
    sheet_id = "underlying_report"
    sheet_title = "Underlying - Report"
    ...
    def __init__(self, master: tk.Misc) -> None:
        ...
        # same concept as in DayReportSheet:
        # precomputed EOD last row per (date, underlying)
        self._master: pd.DataFrame = pd.DataFrame()
        self._available_dates: List[str] = []

        # final displayed dataframe (aggregated per underlying, incl ALL)
        self._df_underlying: pd.DataFrame = pd.DataFrame()
    ...
    def _build(self) -> None:
        ...
        ttk.Label(top, text="Underlying - Report", style="Title.TLabel").pack(side="left")
    ...
    def _clear_days(self) -> None:
        ...
        self._set_underlying_df(pd.DataFrame())
    ...
    def _rebuild_master_if_ready(self) -> None:
        ...
        last = df.loc[
            idx,
            ["date", "underlyingName", "portfolio", "feesCum", "PnLVonDeltaCum", "PremiaCum", "Total"],
        ].copy()
    ...
    def _apply_days(self) -> None:
        ...
        agg = self._aggregate_underlyings(view)
        all_row = self._build_all_row(agg)
        out = pd.concat([agg, all_row], ignore_index=True)

        self._sort = _SortState()
        self._set_underlying_df(out)
        ...
        self.info_var.set(f"Days: {len(sel_days)} | underlyings: {len(agg):,} (+ALL)")
        self.export_btn.configure(state="normal")

    def _aggregate_underlyings(self, df: pd.DataFrame) -> pd.DataFrame:
        work = df.copy()
        ...
        grouped = (
            work.groupby("underlyingName", sort=False, dropna=False)
            .agg(
                feesCum=("feesCum", "sum"),
                PnLVonDeltaCum=("PnLVonDeltaCum", "sum"),
                PremiaCum=("PremiaCum", "sum"),
                Total=("Total", "sum"),
                trades=("trades", "sum"),
                portfolio=("portfolio", self._agg_portfolio),
            )
            .reset_index()
        )

        grouped["trades"] = pd.to_numeric(grouped["trades"], errors="coerce").fillna(0).round().astype(int)

        for c in ["feesCum", "PnLVonDeltaCum", "PremiaCum", "Total"]:
            grouped[c] = pd.to_numeric(grouped[c], errors="coerce").fillna(0.0)

        grouped = grouped[self.REPORT_COLS].sort_values(["underlyingName"], kind="mergesort").reset_index(drop=True)
        return grouped

    @staticmethod
    def _agg_portfolio(s: pd.Series) -> str:
        vals = [str(x).strip() for x in s if pd.notna(x) and str(x).strip()]
    ...
    def _build_all_row(self, df: pd.DataFrame) -> pd.DataFrame:
        sums = {
            c: float(pd.to_numeric(df[c], errors="coerce").fillna(0.0).sum())
            for c in self.NUMERIC_COLS
            if c in df.columns
        }
        if "trades" in sums:
    ...
    # Table render + sort
    # -------------------------
    def _set_underlying_df(self, df: pd.DataFrame) -> None:
        self._df_underlying = df if df is not None else pd.DataFrame()
        self._cache, self._cache_len = build_display_cache(self._df_underlying)
        self._render()

    def _render(self) -> None:
        self.tree.delete(*self.tree.get_children())
        df = self._df_underlying
        if df is None or df.empty:
            return
...
<title>Underlying Report { _html.escape(title_label) }</title>
<style>
  :root {{
    --panel: #ffffff;
    --grid: #e5e7eb;
...
"""
        ...
        default_name = "underlying_report.html"
        if len(sel_days) == 1:
            default_name = f"underlying_report_{sel_days[0]}.html"
        elif len(sel_days) > 1:
            default_name = f"underlying_report_{sel_days[0]}_to_{sel_days[-1]}.html"
        ...
        path = filedialog.asksaveasfilename(
            ...
            title="Save Underlying Report HTML",
        )
        if not path:
            return
